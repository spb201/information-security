module Hash (md5, hashToInteger, hashToString) where
...
hash :: Mode -> B.ByteString -> Digest
hash mode input =
    foldl' foldingFunction initDigest chunks
  where
    message = fancyPad input
...
hashToInteger :: Digest -> Integer
hashToInteger (a, b, c, d) =
    ify 96 a + ify 64 b + ify 32 c + ify 0 d
  where
    ify iary = (`shift` iary) . fromIntegral

hashToString :: Digest -> String
hashToString (a, b, c, d) =
    ify a ++ ify b ++ ify c ++ ify d
  where
    ify x' =
        [ify' 24 x, ify' 16 x, ify' 8 x, ify' 0 x]
      where
        x = fromIntegral x'
        ify' n y =
            chr ((y `shiftR` n) .&. 0xff)
